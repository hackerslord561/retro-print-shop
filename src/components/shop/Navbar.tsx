...
export default function Navbar() {
    ...
    return (
        <nav className="sticky top-0 z-40 bg-retro-cream border-b-4 border-retro-denim px-4 md:px-8 py-3 md:py-4 shadow-xl">
            <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-2 md:gap-4">

                {/* Logo */}
                <Link href="/" className="flex items-center gap-2 group flex-shrink-0">
                    <div className="bg-retro-denim text-white p-1.5 md:p-2 rounded-sm group-hover:bg-retro-ink transition-colors">
                        <Shirt className="w-5 h-5 md:w-6 md:h-6" />
                    </div>
                    <div>
                        <h1 className="text-xl md:text-2xl font-black text-retro-ink tracking-tighter leading-none">
                            OMNI VINTAGE
                        </h1>
                        <p className="text-[8px] md:text-[10px] font-bold text-retro-denim tracking-[0.3em] uppercase">
                            EST. 2026
                        </p>
                    </div>
                </Link>
                ...
                <div className="flex items-center gap-4 md:gap-6">
                    ...
                    >
                        <ShoppingBag className="w-5 h-5 md:w-6 md:h-6 text-retro-ink" />
                        {itemCount > 0 && (
            ...
            <div className="w-full mt-3 md:hidden">
                <form onSubmit={handleSearch} className="relative">
                    <input
                        ...
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full pl-4 pr-10 py-2.5 bg-white border-2 border-retro-denim/20 rounded-lg focus:outline-none focus:border-retro-denim text-sm"
                    />
